...
2. Delete the compiled alias/service dictionaries (store.py:103-140). Alias lookup is deterministic traversal of stored relationships only (section 12). Regression: "system design" no longer resolves "API architecture" as exact_match absent a stored, confirmed relationship.
...
- Dictionary-removal regressions: the fixture pairs previously satisfied by compiled aliases (aws/graphql/node/postgres, "system design"→"API architecture") no longer match without stored relationships.
...
2. Delete the compiled alias/service dictionaries; stored-relationship alias lookup.

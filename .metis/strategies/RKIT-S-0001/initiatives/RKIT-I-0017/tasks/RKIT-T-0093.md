...
- 2026-08-17: T-0092 committed (resume extraction through the adapter, regex engine deleted grep-proof, typed errors on failure, smoke fixtures pinned; gates 476/smoke/verify green; driver probed unpinned-input → provider_error no-partial). Codex launched on the job side following T-0092's exact pattern: engine deletion, Python+Spark and GraphQL+API regressions, model-sourced classification/seniority/industries/domains.
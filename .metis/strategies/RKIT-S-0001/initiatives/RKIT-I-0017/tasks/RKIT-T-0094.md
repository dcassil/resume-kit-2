...
- 2026-08-17: T-0093 committed (job extraction through the adapter, closed engine deleted grep-proof, Python+Spark/GraphQL+API regressions, smoke job fixtures pinned, CLI preferred[] fold-through; gates 480/smoke/verify green; driver probed 8 requirement proposals on job-a + unpinned→typed error). Codex launched on the close-out: uncertainty-grep + hardcoded-confidence deletion, "unscored" placeholders for I-0018/0019-scoped functions, uncertain-item surfacing golden, TEST_SPEC generalization matrix, mutation probes.